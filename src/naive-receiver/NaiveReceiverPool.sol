...
contract NaiveReceiverPool is Multicall, IERC3156FlashLender {
    ...
    // sending a call from the forwarder that has the address of someone other than the msg.sender encoded into the function call allows us to spoof that call as any address
    // we will use this to trick this function into calling withdraw from the deployer address even though we will be calling the forwarder as the player address
    // this is because it only reads the last 20 bytes which we can load with the deployer address when building the BasicForwarder.Request struct
